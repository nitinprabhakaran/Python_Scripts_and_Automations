...
# Make sure you add the db name and password and username, also specify hist IPs in case if you are running instance on a remote machine
def connect():
    try:
        connection = psycopg2.connect(user='any_username', password='any_pass', host='127.0.0.1', database='password_manager')
        return connection
    except (Exception, psycopg2.Error) as error:
        print(error)
